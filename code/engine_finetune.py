...
def misc_measures(confusion_matrix):
    ...
    for i in range(1, confusion_matrix.shape[0]):
        ...
        if denominator != 0:
            mcc = (cm1[0, 0] * cm1[1, 1] - cm1[0, 1] * cm1[1, 0]) / denominator
            mcc_.append(mcc)
        else:
            mcc_.append(0)
...
def evaluate(data_loader, model, device, task, epoch, mode, num_class):
    ...
    if global_rank == 0 and not os.path.exists(task):
        os.makedirs(task)
    ...
    if global_rank == 0:
        with open(results_path,mode='a',newline='',encoding='utf8') as cfa:
            wf = csv.writer(cfa)
            data2=[["acc","sensitivity","specificity","precision","auc_roc","auc_pr","F1","mcc","metric_logger.loss"],[acc,sensitivity,specificity,precision,auc_roc,auc_pr,F1,mcc,metric_logger.loss]]
            for i in data2:
                wf.writerow(i)
